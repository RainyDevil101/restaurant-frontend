...
export function useCatalogFreshness(resources: CatalogResourceKey[] = ['products', 'categories', 'menus']) {
  ...
  async function refreshStale(): Promise<void> {
    ...
    await Promise.all(resources.filter((key) => stores[key].isStale).map((key) => stores[key].refresh()))
  }

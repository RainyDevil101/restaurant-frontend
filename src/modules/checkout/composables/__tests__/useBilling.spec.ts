...
describe('useBilling — with delivered orders', () => {
  ...
  it('billLines aggregates the same product across orders', async () => {
    vi.mocked(listOrdersByTable).mockResolvedValue([order({ id: 'ord-1' }), order({ id: 'ord-2' })])
  ...
  it('keeps distinct products as separate lines', async () => {
    vi.mocked(listOrdersByTable).mockResolvedValue([
      order({
        id: 'ord-1',
        items: [
          { itemId: 'i1', productId: 'p1', productName: 'Tacos', quantity: 1, unitPrice: 90, subtotal: 90 },
          { itemId: 'i2', productId: 'p2', productName: 'Agua', quantity: 2, unitPrice: 20, subtotal: 40 },
        ],
      }),
    ])
    const { billLines, billTotal } = await withSetup()

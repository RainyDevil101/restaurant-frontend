...
describe('useReceiptSettings', () => {
  ...
  it('surfaces the ApiRequestError message when the load fails', async () => {
    vi.mocked(getReceiptSettings).mockRejectedValue(new ApiRequestError('servidor caído', 500, null))
  ...
  it('save shows an error toast and rethrows when the API fails', async () => {
    vi.mocked(updateReceiptSettings).mockRejectedValue(new ApiRequestError('no autorizado', 401, null))
